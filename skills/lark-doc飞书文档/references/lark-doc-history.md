...
`entries[].edit_time` 是 RFC3339 时间字符串（例如 `<REDACTED_DATE>`）。按时间匹配时先将其解析为时间值，再比较先后关系或时间差。
...
- history_version_id=11 revision_id=42 edit_time=<REDACTED_DATE> name=...
- history_version_id=12 revision_id=42 edit_time=<REDACTED_DATE> name=...
```
