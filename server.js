...
const SKILLS_DIR = process.env.SKILLS_DIR_PATH || path.join(ROOT, 'skills');
...
const TOOL_EXECUTION_POLICIES = Object.freeze({
  ...
  capture_metric_snapshot: { maxConcurrency:2 },
});
...
const CLI_REGISTRY = Object.freeze({
  'lark-cli': Object.freeze({
    executable:'lark-cli', cwd:ROOT, timeout:60_000, riskLevel:'external_write',
    capabilities:'飞书文档、云盘、日历、任务等能力。先使用 lark-cli --help 或对应子命令 --help 查询精确参数；命令可能直接修改飞书内容。',
    pathArgs:[{ flag:'--file', root:ASSET_DIR }],
    artifactRules:[],
  }),
  'libra-cli': Object.freeze({
    executable:LIBRA_CLI, cwd:ROOT, timeout:150_000, riskLevel:'external_write',
    capabilities:'Libra 实验、指标、截图和实验配置能力。先使用 libra-cli --help 或对应子命令 --help 查询精确参数；命令可能创建本地截图或修改外部实验配置。',
    pathArgs:[{ flag:'--output', root:ASSET_DIR }],
    artifactRules:[{
      id:'image_output', outputFlag:'--output', type:'image', mimeType:'image/png', managedRoot:ASSET_DIR, registerMedia:true,
      title:'Libra CLI 图片产物',
    }],
  }),
});
...
const ALL_AGENT_TOOLS = [
  ...
  { type:'function', function:{ name:'execute_cli', description:'执行已注册外部 CLI 的明确 argv。不得传可执行路径或 shell 字符串；需要确认命令或参数时，调用 inspect_cli_help 查询真实说明。项目中已有的相关 Markdown Skill 可按需通过 search_skills 查询。argv 会原样作为参数数组传入，不使用 shell。外部写操作会直接执行并留存审计。一次需要执行多个彼此独立的 CLI 命令时，应在同一条 assistant message 中分别调用本工具，服务端会按受控并发批量执行；后一条命令若依赖前一条的输出、文件或远端写入结果，必须等下一轮取得真实 Result 后再调用。受管路径参数必须使用项目内相对路径：libra-cli 的 --output 必须写为 data/assets/<filename>，不能只传文件名、./文件名或绝对路径；lark-cli 的 --file 也必须位于 data/assets/。需要把已导出的图片写入飞书文档时，直接调用 lark-cli 的文档插图命令，并把同一 data/assets/<filename> 作为 --file 传入；无需额外的图片引用参数。', parameters:{type:'object',properties:{cli:{type:'string',enum:Object.keys(CLI_REGISTRY)},argv:{type:'array',minItems:1,maxItems:80,items:{type:'string'}}},required:['cli','argv'],additionalProperties:false} } },
  ...
  { type:'function', function:{ name:'read_web_page', description:'只读抓取一个明确指定的公开 HTTP / HTTPS 网页或文本数据 URL，提取标题和正文节选。仅适用于无需登录的外部网页；会拒绝本机、内网、保留地址和非文本响应，并在重定向后再次校验目标。飞书 Docx/Wiki 请使用 read_feishu_document；Libra 实验请使用对应 Libra 工具。不会保存知识、不会写入外部系统。', parameters:{type:'object',properties:{url:{type:'string'}},required:['url'],additionalProperties:false} } },
...
const AGENT_TOOLS = Object.freeze(ALL_AGENT_TOOLS.filter((tool) => !LEGACY_AGENT_TOOL_NAMES.has(tool.function.name)));
...
const SERVER_MANAGED_STATE_KEYS = Object.freeze(['agentRuns','agentPlans','dataRecoveryReports','artifacts','mediaAssets','cliHelpCache','skills','automationRules','automationPlans','automationTasks','notifications','longTermMemories']);
...
async function captureLibraMetricSnapshot(state, projectId, call) {
  ...
  try {
    ...
    await runArtifactCommand(LIBRA_CLI, args, { timeout:150000, cwd:ROOT });
    const file = await fs.stat(path.join(ROOT, relativeOutput));
...
function resolveChatModel(modelId = '') {
  ...
  if (!model) throw Object.assign(new Error('所选模型不在当前允许的模型列表中。'), { statusCode:422 });
  return model;
}
...
function normalizeModelRequestMockModels(payload) {
  ...
  const known = new Map(CHAT_MODEL_CATALOG.map((model) => [model.id, { id:model.id, label:model.label, supportsVision:model.supportsVision, source:'configured' }]));
  ...
  return [...known.values()].sort((a, b) => a.source === b.source ? a.label.localeCompare(b.label) : a.source === 'configured' ? -1 : 1);
}
...
async function fetchModelRequestMockModels({ fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  if (!OPENROUTER_API_KEY) return { models:fallback, source:'configured' };
  ...
  try {
    const response = await fetchImpl(`${OPENROUTER_BASE_URL.replace(/\/$/,'')}/models?output_modalities=text&sort=most-popular`, {
      headers:{ Authorization:`Bearer ${OPENROUTER_API_KEY}`, 'Content-Type':'application/json' }, signal:controller.signal,
    });
    const payload = await response.json().catch(() => ({}));
    ...
    return { models, source:response.ok ? 'provider' : 'configured' };
  } catch {
    return { models:fallback, source:'configured' };
  } finally {
    clearTimeout(timer);
  }
}
...
function modelRequestToolCatalog() {
  const unrestricted = new Set(AGENT_TOOLS.map((tool) => tool.function.name));
  return ALL_AGENT_TOOLS.map((tool) => ({
    name:tool.function.name,
    description:String(tool.function.description || ''),
    defaultSelected:unrestricted.has(tool.function.name),
    availability:unrestricted.has(tool.function.name) ? 'unrestricted' : 'restricted',
  }));
}
...
function selectModelRequestTools(selectedToolNames) {
  ...
  const available = new Set(ALL_AGENT_TOOLS.map((tool) => tool.function.name));
  ...
  return ALL_AGENT_TOOLS.filter((tool) => requested.has(tool.function.name));
}
...
function agentInstructions() {
  return [
    ...
    '用户提供明确 URL 且要求总结、核实、提意见或补全内容时，先按来源读取真实内容：飞书 Docx/Wiki 用 read_feishu_document，公开 HTTP/HTTPS 网页用 read_web_page，Libra 实验用对应 Libra 工具；读取失败必须披露，不能假装已读。',
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === '/api/health' && request.method === 'GET') {
      return sendJson(response, 200, { ok: true, storage: 'json-file' });
    }

    if (url.pathname === '/api/ai/models' && request.method === 'GET') {
      return sendJson(response, 200, { models:CHAT_MODEL_CATALOG, defaultModel:resolveChatModel().id });
    }
    ...
    if (url.pathname === '/api/media-assets/paste' && request.method === 'POST') {
      ...
      await fs.writeFile(path.join(ROOT, filePath), image.buffer);
...
async function resolveInitiativeMediaAssets(state, { projectId, initiativeId, imageAssetIds }) {
  ...
  for (const id of requested) {
    ...
    const file = await fs.stat(path.join(ROOT, relativePath)).catch(() => null);
...
async function resolveChatImageAttachments(state, { projectId = '', initiativeId = '', conversationId = '', imageAssetIds }) {
  ...
  for (const id of requested) {
    ...
    const relativePath = relativeManagedAssetPath(asset.filePath);
    const buffer = await fs.readFile(path.join(ROOT, relativePath)).catch(() => null);
...
async function replaceFeishuDocumentImages({ documentUrl, replacements, assets }) {
  ...
  for (const item of replacements || []) {
    ...
    try {
      const { stdout } = await execFileAsync('lark-cli', ['docs','+media-insert','--as','user','--doc',documentUrl,'--file',asset.filePath,'--selection-with-ellipsis',anchorText,'--before','--align','center','--caption',String(item.caption || asset.filename || ''),'--format','json'], { timeout:60000, maxBuffer:2*1024*1024, cwd:ROOT });
...
async function insertDocumentImages({ documentUrl, assets }) {
  ...
  for (const asset of assets) {
    try {
      const { stdout } = await execFileAsync('lark-cli', ['docs','+media-insert','--as','user','--doc',documentUrl,'--file',asset.filePath,'--align','center','--caption',asset.filename,'--format','json'], { timeout:60000, maxBuffer:2*1024*1024, cwd:ROOT });
