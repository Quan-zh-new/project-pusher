...
let initialProjectTableRendered = false;
...
function normalizeState() {
  ...
  if (typeof state.hasSelectedChatModel !== 'boolean') state.hasSelectedChatModel = false;
  if (!Array.isArray(state.quickPrompts)) state.quickPrompts = [];
  state.quickPrompts = [...new Set(state.quickPrompts.map((item) => String(item || '').trim()).filter((item) => item && item.length <= 240))].slice(0, 12);
...
async function hydrateDemoMode() {
  if (!API_ENDPOINT) return;
  try {
    const response=await fetch('/api/demo/config',{cache:'no-store'});
    const payload=await response.json();
    hostedDemoMode=Boolean(payload.enabled);
    document.body.classList.toggle('hosted-demo', hostedDemoMode);
    document.getElementById('demoBanner').hidden=!hostedDemoMode;
  } catch {}
}
async function resetHostedDemo() {
  if (!hostedDemoMode || !confirm('恢复全站的初始演示数据？当前演示中的修改会被清除。')) return;
  const response=await fetch('/api/demo/reset',{method:'POST'});
  const payload=await response.json();
  if (!response.ok) return toast(payload.error||'恢复失败');
  state=payload.state; normalizeState(); saveAndRender('已恢复初始演示数据');
}
async function hydrateChatModelCatalog() {
  ...
  const selectedModelAvailable=chatModelCatalog.some((item)=>item.id===state.selectedChatModel);
  if ((!state.hasSelectedChatModel && payload.demo) || !selectedModelAvailable) state.selectedChatModel=payload.defaultModel || chatModelCatalog[0]?.id || '';
}
...
function renderAll() { renderShell(); renderNotifications(); renderCounts(); renderDeadlines(); renderProjectTable(); const activeView=document.querySelector('.view.active-view')?.id || 'projects'; if(activeView==='setup')renderSetup(); if(activeView==='knowledge'){renderKnowledgeFilters();renderKnowledge();} if(activeView==='automation')renderAutomation(); if(activeView==='cli-skills')renderCliSkills(); if(activeView==='project-memory')renderProjectMemory(); if(activeView==='agent-runs')renderAgentRuns(); renderAssistant(); autoSizeInlineTextareas(); }
...
async function refreshSetupStatus() { const root=document.getElementById('setupStatus'); if(!API_ENDPOINT){setupStatus={offline:true};renderSetup();return;} if(root)root.innerHTML='<div class="empty-state compact">正在检查本机配置…</div>'; try { const response=await fetch('/api/setup/status',{cache:'no-store'}); const payload=await response.json(); if(!response.ok)throw new Error(payload.error||'配置检查失败'); setupStatus=payload; renderSetup(); } catch(error) { setupStatus={error:error.message}; renderSetup(); } }
async function saveOpenRouterKey(form) { const input=form.elements.apiKey; const apiKey=String(input?.value||'').trim(); if(!apiKey)return toast('请输入 OpenRouter API Key'); const submit=form.querySelector('button[type="submit"]'); if(submit)submit.disabled=true; try { const response=await fetch('/api/setup/openrouter-key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({apiKey})}); const payload=await response.json(); if(!response.ok)throw new Error(payload.error||'保存失败'); input.value=''; await hydrateChatModelCatalog(); await refreshSetupStatus(); renderAssistant(); toast('API Key 已保存到本机 .env，并已在当前服务生效'); } catch(error) { toast(error.message||'保存失败'); } finally { if(submit)submit.disabled=false; } }
function renderSetup() { const root=document.getElementById('setupStatus'); if(!root)return; if(setupStatus?.error){root.innerHTML=`<div class="run-step-error">无法读取配置状态：${escapeHtml(setupStatus.error)}</div>`;return;} if(setupStatus?.offline){root.innerHTML='<div class="empty-state compact">当前是直接打开 HTML 的离线模式。请运行 <code>npm start</code> 后刷新页面，以使用本地后端和 AI。</div>';return;} if(!setupStatus){root.innerHTML='<div class="empty-state compact">打开“开始使用”时会自动检查配置。</div>';return;} const item=(label,ok,detail,optional=false)=>`<div class="setup-check ${ok?'ready':'pending'}"><i>${ok?'✓':optional?'○':'!'}</i><span><b>${label}</b><small>${escapeHtml(detail)}</small></span><em>${ok?'就绪':optional?'可选':'待配置'}</em></div>`; const installedCli=Array.isArray(setupStatus.cli?.installed)?setupStatus.cli.installed:[]; const checks=[item('本地服务',setupStatus.server?.running,`正在 ${setupStatus.server?.host}:${setupStatus.server?.port} 运行`),item('OpenRouter API',setupStatus.ai?.configured,setupStatus.ai?.configured?`已配置 · ${setupStatus.ai.model}`:'复制 .env.example 为 .env 后填入 API Key'),...installedCli.map(cli=>item(`CLI · ${cli.name}`,true,cli.capabilities||`命令：${cli.command}`,true))]; const cliNote=installedCli.length?`<p class="setup-cli-note">已发现 ${installedCli.length} 个已安装的已接入 CLI。</p>`:'<p class="setup-cli-note">未发现已接入的外部 CLI；这不影响项目管理、对话和文档体验。</p>'; root.innerHTML=`<div class="setup-checks">${checks.join('')}</div>${cliNote}${setupStatus.server?.demo?'<p class="setup-demo-note">当前为演示模式：不会调用真实 AI、飞书或 Libra 账号。</p>':''}`; }
...
function renderProjectTable() { const body = document.getElementById('projectTable'); const knownProjectIds=new Set(state.projects.map(project=>project.id)); state.expandedProjectIds=state.expandedProjectIds.filter(id=>knownProjectIds.has(id)); if (!initialProjectTableRendered && state.projects.length && !state.expandedProjectIds.length) state.expandedProjectIds=state.projects.map(project=>project.id); initialProjectTableRendered=true; const rows = [...state.projects].sort((a,b) => priorityRank(a.priority) - priorityRank(b.priority) || a.name.localeCompare(b.name, 'zh-CN')).map(project => {
...
function renderQuickPrompts() { const root=document.getElementById('quickPrompts'); if(!root)return; const saved=(state.quickPrompts||[]).map((prompt,index)=>`<div class="quick-prompt-row"><button type="button" data-ai-prompt="${escapeHtml(prompt)}">${escapeHtml(prompt)}</button><button type="button" class="quick-prompt-delete" data-delete-quick-prompt="${index}" title="删除快捷提示" aria-label="删除快捷提示">×</button></div>`).join(''); root.innerHTML=`<button type="button" class="quick-prompt-trigger" data-toggle-quick-prompts aria-expanded="${quickPromptMenuOpen}">快捷提示 <i>⌄</i></button>${quickPromptMenuOpen?`<div class="quick-prompt-menu">${saved}${customQuickPromptOpen?`<label><input id="customQuickPrompt" type="text" maxlength="240" placeholder="输入快捷提示" /><button type="button" data-submit-ai-prompt>保存并发送</button></label>`:`<button type="button" class="quick-prompt-add" data-add-ai-prompt>＋ 自定义</button>`}</div>`:''}`; }
...
document.addEventListener('click', event => {
  if (event.target.id === 'resetHostedDemo') { void resetHostedDemo(); return; }
  if (event.target.closest('[data-refresh-setup]')) { void refreshSetupStatus(); return; }
  const copyCommand=event.target.closest('[data-copy-command]'); if(copyCommand){ const command=copyCommand.dataset.copyCommand; if(navigator.clipboard?.writeText)navigator.clipboard.writeText(command).then(()=>toast('命令已复制到剪贴板')).catch(()=>toast(`请手动复制：${command}`)); else toast(`请手动复制：${command}`); return; }
  ...
  const deleteQuickPrompt=event.target.closest('[data-delete-quick-prompt]'); if(deleteQuickPrompt){state.quickPrompts.splice(Number(deleteQuickPrompt.dataset.deleteQuickPrompt),1);saveState();renderQuickPrompts();return;}
  if(event.target.closest('[data-submit-ai-prompt]')){const input=document.getElementById('customQuickPrompt');const prompt=input?.value.trim();if(prompt){state.quickPrompts=[prompt,...state.quickPrompts.filter(item=>item!==prompt)].slice(0,12);saveState();quickPromptMenuOpen=false;customQuickPromptOpen=false;renderQuickPrompts();aiReply(prompt);}return;}
...
if (API_ENDPOINT) { void hydrateDemoMode(); hydrateFromBackend(); }
else renderAll();
