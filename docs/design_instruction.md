...
External optional dependencies
  -> OpenRouter, libra-cli, lark-cli, public-link fetch
```
...
- **外部适配层**：OpenRouter、Libra、飞书均从后端调用。外部结果必须转换为本地统一的 Tool Result/Artifact 记录；密钥、认证头和本机 CLI 缓存不得进入浏览器或状态文件。
...
飞书文档和 Libra 原子工具直接进入 `AGENT_TOOLS`，由已有参数校验、Run 审计、Plan 和外部读回约束执行；不再要求模型先猜测 `lark-cli`/`libra-cli`。新增只读 `read_web_page(url)`：仅读取公开 HTTP/HTTPS 文本页面，逐跳拒绝本机、内网和保留地址，限制重定向、响应类型与正文长度；飞书 Docx/Wiki 必须走 `read_feishu_document`，Libra 实验必须走其专用工具。`query_agent_history` 仍保持不对模型开放。
